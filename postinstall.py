...
with open('postinstall.pkgs') as pkgs:
    while pkgs.readable():
        for line in pkgs:
            os.system(f'pacman -S {line.strip()} --no-confirm')
...
def install_apps():
    ...
    os.system("pacman -U PKGBUILDs/packages/oosle-shortcuts-1.0-1-any.pkg.tar.zst --no-confirm")
    os.system("pacman -U PKGBUILDs/packages/turbowarp-desktop-bin-1.8.1-1-any.pkg.tar.zst --no-confirm")
